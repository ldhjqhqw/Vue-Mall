        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <!-- saleAttr是每一个属性 saleAttr.spuSaleAttrValueList是某个属性下面多有的属性值列表  attrValue是每一个属性值-->
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                <dt class="title">{{ saleAttr.saleAttrName }}</dt>
                <dd
                  @click="changeChecked(attrValue,saleAttr.spuSaleAttrValueList)"
...
export default {
  ...
  methods: {
    ...
    changeChecked(attrValue,spuSaleAttrValueList){
      // 拿到某个属性所有的属性值数组 让每一个人的isChecked为0
    ...
    // 点击添加购物车
    async addShopCart(){
      // 应该向后端发送请求 让后端在数据库存储起来某些商品信息 
      // dispatch本质就是在掉调用给某个actions函数
      // dispatch的返回值其实就是actions函数的返回值
      try {
        await this.$store.dispatch('getAddShopcart',{skuId:this.skuId,skuNum:this.skuNum})
        // 储存成功 跳转到添加购物车成功页面
        // 跳转到添加购物车页面时 咱们需要 传递一些数据(名字 价格 数量 属性及属性值 图片 id)
        let goodsInfo = {
        ...
        this.spuSaleAttrList.forEach(attr=>{
          // 去找到这个属性下面选中的那个属性值
          let checkedValue =  attr.spuSaleAttrValueList.find(attrValue=>{
            return attrValue.isChecked === '1'
          })
          // 拼个字符串  属性名:属性值的名字
          let str = `${attr.saleAttrName}:${checkedValue.saleAttrValueName}`
          // 放入attrs中
          goodsInfo.attrs.push(str)
        })
        // 咱们并不把数据做为params或者query的形式传递过去 不是因为丑
        // params参数和query参数是有长度限制的 如果传递一些简单数据 用params和query 传递复杂数据 sessionStorage(临时数据中转站)
